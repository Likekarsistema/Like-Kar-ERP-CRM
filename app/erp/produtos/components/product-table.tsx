...
export function ProductTable({
  ...
  const handleStockClick = (productId: string) => {
    router.push(`/erp/movimentacoes?produto=${productId}`)
  }
          ...
          <TableBody>
            {products.map((product) => (
              <TableRow key={product.id} className="hover:bg-gray-50">
                ...
                <TableCell>
                  <div>
                    <Link href={`/erp/produtos/${product.id}`} className="font-medium text-gray-900 cursor-pointer">
                      {product.name}
                    </Link>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem asChild>
                        <Link href={`/erp/produtos/${product.id}`}>
                          <Eye size={16} className="mr-2" />
                          Ver Detalhes
                        </Link>
                      </DropdownMenuItem>
                      <DropdownMenuItem asChild>
                        <Link href={`/erp/movimentacoes?produto=${product.id}`}>
                          <History size={16} className="mr-2" />
                          Histórico de Estoque
                        </Link>
                      </DropdownMenuItem>
                      <DropdownMenuItem asChild>
                        <Link href={`/erp/produtos/${product.id}/editar`}>
